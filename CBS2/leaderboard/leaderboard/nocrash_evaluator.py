    ...
    def _cleanup(self):
        ...
        for i, _ in enumerate(self.ego_vehicles):
            if self.ego_vehicles[i]:
                logging.info(f"Destroying ego vehicle: {self.ego_vehicles[i].id}")
                self.ego_vehicles[i].destroy()
                self.ego_vehicles[i] = None
        self.ego_vehicles = []

        if hasattr(self, 'agent_instance') and self.agent_instance:
            logging.info("Destroying agent instance")
        ...
        # Log remaining actors
        remaining_actors = self.world.get_actors()
        for actor in remaining_actors:
            logging.info(f"Remaining actor after cleanup: {actor.id}, type: {actor.type_id}")
